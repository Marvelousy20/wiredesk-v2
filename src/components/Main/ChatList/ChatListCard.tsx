...
const ChatListCard: React.FC<{ messages: ChatMessage[] }> = ({ messages }) => {
  return (
    <ul className="space-y-2">
      {messages.map((msg) => (
        <li key={msg.id} className="flex items-center p-2 rounded-[10px]">
          ...
          <div className="flex-grow">
            ...
            <p
              className={`truncate text-xxs mt-1.5 ${
                msg.isUnread ? "font-semibold" : "font-medium text-[#37352FBF]"
              }`}
            >
              {msg.message}
            </p>
          </div>
          <div className="flex flex-col items-end justify-end gap-y-2 h-[40px]">
            {msg.isUnread && (
